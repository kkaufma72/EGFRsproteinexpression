...
P.T. = Modifications by Pablo Tamayo
 - Changed the updates for sigma and the learning rate (asymptotic decay)
 - Added parameters to provide initial sigma and random seed
"""
...
## P.T
import pcal.hexagons as hx
import pcal.densityPeak as dp
import pcal.qualityThreshold as qt
...
class somNet:
    ...
    def __init__(self, netHeight, netWidth, data, loadFile=None, PCI=0, PBC=0, n_jobs=-1, rseed=63574):
        ...
        """P.T. set seed for random number generator."""
        ...
        """ P.T. we change np.float's to float's 3/18/2024 """
    ...
    def update_sigma(self, iter):
        ...
        self.sigma = self.startSigma / (1 + iter/(self.tau/2))
    ...
    def update_lrate(self, iter):
        ...
        self.lrate =  self.startLearnRate / (1 + iter/(self.tau/2))
...
#        minVal=np.finfo(np.float).max
    ...
    def train(self, startLearnRate = 0.01, epochs=-1, startSigma = 1.0):
        ...
        for i in range(self.epochs):
            ...
            """
            ### P.T. use batch update insraed of sampling with replacement
            #inputVec = self.data[np.random.randint(0, self.data.shape[0]), :].reshape(np.array([self.data.shape[1]]))
            inputVec = self.data[data_index % self.data.shape[0], :].reshape(np.array([self.data.shape[1]]))
    ...
    def nodes_graph(self, colnum=0, show=False, printout=True, path='./', colname=None):
        ...
        if self.colorEx==True:
            ...
            cols = [[float(node.weights[0]), float(node.weights[1]), float(node.weights[2])]for node in self.nodeList]   
